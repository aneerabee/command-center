...
const PRJ = [
  ...
  },
  {
    ...
    subsystems: ["static-site", "caddy-reverse-proxy", "letsencrypt-ssl", "daily-backup"],
    related_services: [],
    related_tools: [],
  ...
  },
  {
    ...
    users_count: 2,
    priority: "high",
    next_milestone: "إعداد قاعدة Supabase + تشغيل migrations + seed بقصة ربيع الحقيقية + إطلاق نسخة محلية",
    summary:
      "دفتر مالي شخصي للشراكة بين ربيع وأحمد — Next.js 15 + Drizzle ORM + Supabase. 5 جداول صارمة، 14 نوع حركة، صفر أتمتة قسرية، أرصدة محسوبة من movements.",
    ...
    current_status: {
      updated: "2026-05-14",
      where:
        "الكود محلي تمامًا في /Users/rabeeshaban/Documents/New project/libya/app/. Next.js 15 + Drizzle + 5 جداول + 14 نوع حركة + seed بقصة ربيع الحقيقية كله جاهز. Migration SQL مولّد. ينتظر فقط مفاتيح Supabase لتشغيل migrate + seed + dev server.",
      next_step:
        "إنشاء مشروع Supabase جديد باسم libya (أو استخدام نفس مشروع Mohammad Ledger مع schema منفصل)، ثم: pnpm db:migrate ثم pnpm db:seed ثم pnpm dev.",
      blockers: [
        {
          text: "لا توجد قاعدة بيانات حية بعد — التطبيق جاهز كليًا لكن لا يستطيع التشغيل بدون DATABASE_URL.",
          priority: "high",
        },
        {
          text: "بوت تيليجرام لم يُبنَ بعد — مخطط للمرحلة الرابعة بعد استقرار الويب.",
          priority: "low",
        },
      ],
      use_guide:
        "للتطوير: cd إلى libya/app ثم pnpm install ثم cp .env.example .env.local ثم املأ DATABASE_URL وSupabase keys. لتشغيل أول مرة: pnpm db:migrate && pnpm db:seed && pnpm dev → http://localhost:3000. لتعديل المخطط: عدّل src/lib/db/schema.ts ثم pnpm db:generate ثم pnpm db:migrate.",
    },
    bot_features: {
      ...
      sections: [
        ...
        {
          title: "ما تم بناؤه فعليًا (المرحلة 1)",
          items: [
            "هيكل Next.js 15 كامل + tsconfig + tailwind v4 + postcss",
            "src/app/globals.css بكل tokens هوية v2",
            "src/app/layout.tsx مع RTL وخطوط Cairo + Fraunces",
            "src/app/page.tsx لوحة 'اليوم' كاملة مربوطة بدوال queries",
            "src/lib/db/schema.ts بـ5 جداول، 6 enums، relations كاملة",
            "src/lib/db/client.ts اتصال postgres-js + drizzle",
            "src/lib/db/queries.ts دوال محسوبة للأرصدة",
            "drizzle/seed.ts بقصة ربيع الحقيقية كاملة (18 حركة)",
            "drizzle/migrations/0000_*.sql مولّد جاهز للتشغيل",
            "package.json بكل التبعيات + scripts للـmigrate/seed/studio",
            ".env.example + .gitignore + README.md بدليل تشغيل عربي",
          ],
        },
        {
          title: "خطة المراحل القادمة",
          items: [
            "المرحلة 2: صفحات CRUD للناس + المواقع + العملات",
            "المرحلة 2: تدفق إنشاء صفقة + تمويلها + استلام الدولار",
            "المرحلة 2: شاشة الإغلاق المرنة (يدوية مع اقتراحات)",
            "المرحلة 3: صفحة شخص بخط زمني كامل",
            "المرحلة 3: تقارير شهرية + بحث ذكي",
            "المرحلة 3: اختصارات لوحة المفاتيح",
            "المرحلة 4: بوت تيليجرام للإدخال السريع",
            "المرحلة 4: نشر على Contabo + libya.rabee.dev عبر Cloudflare Tunnel",
            "المرحلة 4: نسخة احتياطية يومية تلقائية",
          ],
        },
      ],
    },
    claude_session: {
      ...
      note: "آخر نقطة: الأساس مكتمل (schema + seed + dashboard + design system). ينتظر مفاتيح Supabase لتشغيل migrate + seed، ثم البناء يكمل بصفحات الناس/المواقع/الصفقات/الإغلاق.",
    },
  },
];
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        on: true,
        what: "تطبيق LIBYA الكامل على المنفذ 3001، autostart عند الإقلاع",
        prj: ["LIBYA"],
      ...
      },
      {
        ...
        freq: "دائم (systemd user)",
        on: true,
        what: "بوت Telegram لإدخال حركات LIBYA السريع من الجوال",
        prj: ["LIBYA"],
        path: "server:~/.config/systemd/user/libya-bot.service",
        kind: "bot",
      },
      {
        ...
        freq: "يوميًا 4 صباحًا",
        on: true,
        what: "تصدير libya schema إلى .sql.gz في .backup، الاحتفاظ بآخر 30 يوم",
        prj: ["LIBYA"],
...
const BOT = [
  ...
  },
  {
    ...
    uptime_status: "cloud-running",
    check_note: "بوت إدخال حركات سريع لمنظومة LIBYA — يعمل كخدمة systemd user على Contabo، يربط بنفس قاعدة libya schema في Supabase.",
    runtime: "Node.js 20 + grammy + tsx",
    channel: "Telegram",
    related_entities: ["LIBYA", "Supabase", "Contabo VPS"],
    summary:
      "بوت Telegram لإدخال الحركات السريع من الجوال لمنظومة LIBYA. أوامر: /balance، /debts، /deals، /add، /web. allowlist بـTelegram user_id.",
