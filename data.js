...
const PRJ = [
  {
    ...
    links_desc: {
      ...
      GitHub:
        "الكود المصدري — Next.js 16.2 و Supabase وبوت تلجرام و2,088 اختبار يمر",
      Supabase:
    ...
    current_status: {
      updated: "2026-05-15",
      where:
        "النظام حي ومنشور على Railway (verified HTTP 200، آخر deploy 2026-05-15) ومرتبط بالكامل ببوت تلجرام @BrixPrice_bot. البوت يعيد نفس الأسعار من نفس الفنادق بنفس التواريخ التي يعيدها الويب. ١١٤ commit في آخر أسبوعَين. أحدث الإصدارات: حاجز booking-window (يرفض اليوم في الواجهة والـAPI، يلوّن الغد كتحذير برتقالي، توقيت إسطنبول)، تحويل release_days من رفض كامل إلى تحذير أحمر داخل بطاقة العرض نفسها مع شارة inline، إعادة تصميم بطاقات v15.8 (سعر الليلة أبرز رقم، الإجمالي تحته مباشرة، حذف /person)، تعريب أسماء الأشهر في عرض التاريخ في البوت، استمرار مؤشّر الكتابة طوال البحث الطويل، زر إعادة المحاولة بعد فشل مؤقّت. 2,088 اختبار يمر، صفر ثغرات حرجة. ثلاث طبقات حماية لمضاعفات الإشغال: Time Machine داخل الموقع، نسخ يومي على GitHub (brix-backups في 02:00ص)، حماية برمجية ترفض حفظ قائمة فارغة. سوينو ديلوكس مكتمل ١٠٠٪ (٧ غرف، ١٠ فترات، ٧٠ سعر، ٧٥٠ مضاعف، ١ خصم HHHIIITEM 45%) — تأكيد بمقارنة سطر-بسطر ضد ملف CSV الأصلي بتاريخ ٢٠٢٦-٠٥-١٤. ريكسوس مدخل (١٢ غرفة، ٨ فترات، ٩٦ سعر، ٦٥٦ مضاعف، ٤ خصومات LSD/HND/EXD) مع ٣ تصحيحات صغيرة معروفة (تقريب سعر في ٤ غرف، Deluxe Terrace infant flag، قابلية دمج LSD/HND مع EBD مستقبلاً). ملاحظة: الـREADME يذكر vercel.app كهدف لكن لم يحدث الانتقال — لا يوجد vercel.json في الـrepo والـURL يعطي 404.",
      next_step:
        "أولاً، تحقق دورياً من أن طبقات الحماية الثلاث ما زالت تعمل. ادخل مستودع بريكس باك آبس على غيت هب وتأكد أن نسخة اليوم موجودة في مجلد ديلي بتاريخ اليوم. لو ما وُجدت اذهب لتاب أكشنز في مستودع بريكس ترافل سيستم واطّلع على آخر تشغيل لـدايلي كونتراكت باك آب. ثانياً، تحقق من شاشة آلة الزمن في الموقع لكل فندق تعدّله، يجب أن ترى صورتين جديدتين بعد كل حفظ. ثالثاً، أصلح ٢٢ قيمة سعة غرف خاطئة في ريكسوس (الجدول يحوي تركيبات maxAdults/maxChildren/maxInfants غير صحيحة لا تؤثر على السعر لكنها تسمح بحجوزات خارج العقد). رابعاً، أدخل بقية العقود الثمانية من ملفات بي دي إف (ماكس رويال، ريغنوم، إيلا، كاليستا، سوسيسي، سيغنوس-دراغون، فوياج، شلهان غولف). خامساً، أعد تفعيل النشر التلقائي GitHub→Railway (معطّل حالياً، النشر يدوي عبر mcp__railway__deploy).",
      blockers: [
        {
        ...
        {
          text: "النشر التلقائي GitHub→Railway معطّل — كل نشر حالياً يدوي عبر سطر الأوامر (mcp__railway__deploy). يحتاج إعادة ربط webhook في إعدادات Railway.",
          priority: "high",
        },
        {
