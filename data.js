...
const PRJ = [
  ...
  },
  {
    ...
    users_count: 1,
    priority: "high",
    next_milestone: "تنفيذ ترحيل brixtravel.com من Hostinger إلى Contabo VPS وفق brixtravel-deploy/RUNBOOK.md",
    summary: "موقع WordPress كان على Hostinger — الاستضافة انتهت والموقع متوقف. قيد الترحيل إلى Contabo VPS عبر Docker (WordPress + MariaDB + Caddy) في /opt/brixtravel. النطاق سليم على Cloudflare DNS، البريد يبقى على Hostinger MX.",
    local_path: "/Users/rabeeshaban/Desktop/Projects/🌐 brixtravelwebsite",
    server_path: "/opt/brixtravel",
    repo_url: "https://github.com/aneerabee/brixtravel",
    deploy_url: "https://www.brixtravel.com",
    stack: ["WordPress 6.7", "PHP 8.3", "MariaDB 11", "Caddy 2", "Docker Compose"],
    subsystems: ["wp-admin", "wp-content", "wp-includes", "assets", "caddy-proxy", "mariadb", "daily-backup"],
    related_services: [],
    related_tools: [],
    related_cloud: ["GitHub", "Cloudflare", "Contabo VPS", "Hostinger"],
    ops: [
      "stack معزول في /opt/brixtravel، لا يلمس Wapy ولا Mohammad Ledger",
      "Caddy يحجز 80/443 ويصدر شهادة Let's Encrypt تلقائياً",
      "Cloudflare SSL Mode يجب أن يصبح Full strict بعد إصدار الشهادة",
      "نسخة احتياطية يومية 03:30 إلى /opt/brixtravel/backups (DB + wp-content)",
      "البريد @brixtravel.com على Hostinger MX — لا يُمَس",
    ],
    desc: "الموقع الرسمي لشركة BRIX TRAVEL\n\n🌍 عربي + إنجليزي · اكتشاف تلقائي للغة\n📱 تصميم متجاوب · Google Maps مدمج\nWhatsApp + Instagram + Facebook\n\n🔧 التقنيات:\nWordPress + PHP + HTML/CSS/JS\n\n📡 الوضع الحالي:\nاستضافة Hostinger منتهية → الموقع متوقف.\nقيد الترحيل إلى Contabo VPS (46.202.172.151) عبر Docker Compose.\nالملفات الجاهزة في brixtravel-deploy/ على فرع claude/restart-brixtravel-server-5ju7a.\nالنطاق سليم على Cloudflare DNS، NS: cris + kira.\n\n🎨 الهوية البصرية:\nBrix Travel.ai + .eps (شعار كامل)\n\n🛠️ يحتاج تطوير لاحقاً:\nواجهة حجز مباشرة مربوطة بـ BRIX Travel System",
    tags: ["WordPress", "PHP", "Contabo", "Docker", "Caddy", "ترحيل من Hostinger"],
    ...
    current_status: {
      updated: "2026-05-14",
      where:
        "متوقف. استضافة Hostinger ملغاة، public_html شبه فارغ، صفحة الإدارة تحوّل إلى buy-hosting. Cloudflare DNS سليم على cris+kira NS، SSL مؤقتاً على Flexible للتشخيص. ملفات WordPress الكاملة محفوظة محلياً على ماك المالك. الـ stack الجديد لـ Contabo جاهز في brixtravel-deploy/ ويتضمن docker-compose + Caddyfile + backup.sh + RUNBOOK.md.",
      next_step:
        "نفّذ خطوات RUNBOOK.md بالترتيب من ماكك: (1) أصلح وصول SSH على Contabo (المفتاح ed25519 يحتاج يُضاف إلى /home/argaz/.ssh/authorized_keys)، (2) ارفع ملفات brixtravel-deploy إلى /opt/brixtravel، (3) ارفع محتوى WordPress عبر rsync، (4) docker compose up، (5) وجّه Cloudflare A records إلى 46.202.172.151 مع DNS only مؤقتاً، (6) انتظر شهادة Let's Encrypt، (7) أعد التفعيل Proxied + Full strict، (8) جدول backup.sh كرون 03:30.",
      blockers: [
        {
          text: "وصول SSH إلى Contabo فشل آخر محاولة (Permission denied publickey). الحل: إضافة المفتاح العام ed25519 إلى /home/argaz/.ssh/authorized_keys عبر Contabo Web Console أو Tailscale — التفاصيل في RUNBOOK.md قسم 1.",
          priority: "high",
        },
        {
          text: "النسخة المحلية من ملفات WordPress على iCloud قد تكون placeholders. شغّل brctl download قبل rsync أو اسحب من Finder.",
          priority: "med",
        },
      ],
    },
...
const CLD = [
  ...
  },
  {
    ...
    id: "contabo-vps",
    active: true,
    active_note: "نشط: يستضيف Wapy.dev وبوت دفتر محمد، وقريباً موقع Brixtravel بعد الترحيل من Hostinger",
    em: "🖥️",
    category: "infrastructure",
    dt: "vmi3061403 · Ubuntu 24 · Tailscale 100.116.69.101",
    ip_note: "Public IP موثّق بقيمتين متضاربتين: 62.171.128.44 (سجل قديم في هذا الملف) و 46.202.172.151 (من جلسة ترحيل Brixtravel). Tailscale IP المؤكد عبر runtime-sync = 100.116.69.101. تحقق من الـ public IP الفعلي قبل أي A record جديد على Cloudflare.",
